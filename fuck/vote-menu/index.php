    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      <header class="demo-header mdl-layout__header mdl-color--grey-100 mdl-color-text--grey-600">
        <div class="mdl-layout__header-row">
          <span class="mdl-layout-title">Vote Menu</span>          
        </div>
      </header>
			...
			<?php
				...
				if(mysqli_num_rows($c)!=0)
				{
					echo "<div class='mdl-card mdl-shadow--2dp mdl-cell mdl-cell--4-col mdl-cell--4-col-tablet mdl-cell--12-col-desktop'>
						  ...
						  <h1 class='mdl-card__title-text'>You've already voted for next week's menu. Stay Calm & wait for next vote</h1>
						  </div></div>";					
				}
				else{
				...
				foreach($db as $d)
				{
					...
					<div>						
						...
						for($i=1;$i<4;$i++)
						{
							$sid=$i+$sum;
							$name=$dayid.' '.$i;
							$ab=mysqli_query($data,"SELECT `foodname` FROM `food` WHERE `foodtype`=$i AND `deleted`=0");
							echo "
							...
							<input class='mdl-textfield__input' type='text' name='fday[$dayid][$i]' id='$sid' value='' required readonly tabIndex='-1'>
							<label for='$sid'>
				...
				echo "<div class='mdl-layout-spacer'><br></div><button class='mdl-button mdl-color--accent mdl-color-text--accent-contrast mdl-button--raised mdl-js-button mdl-js-ripple-effect' type='submit' name='addlunch'>Vote</button></form>";
				}
			?>
        </div><br>
      </main>	  
